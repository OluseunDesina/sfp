...
@Component({
  selector: "app-bulk-carterer-upload",
  templateUrl: "./bulk-carterer-upload.component.html",
  styleUrls: ["./bulk-carterer-upload.component.scss"],
  animations: [
...
export class BulkCartererUploadComponent implements OnInit {
  title = "Upload Caterer";
  form: FormGroup;
  ...
  uploadStaffsInfo() {
    this.userService.uploadCarterer({
      ...
      farmers: [],
      students: [],
      caterers: this.htmlWorkbookJSON.map(item => ({
        firstname: `${item["CATERER FIRST NAME"]}`,
        middlename: `${item["CATERER MIDDLE NAME"]}`,
        lastname: `${item["CATERER LAST NAME"]}`,
        nin: `${item["CATERER NIN"]}`,
        state: `${item["STATE"].toUpperCase()}`,
        city: `${item["TOWN"].toUpperCase()}`,
        lga: `${item["LG"].toUpperCase()}`,
        ward: `${item["WARD"].toLowerCase()}`,
        address: `${item["CATERER ADDRESS"].toUpperCase()}`,
        is_approved: true
      })),
    });
